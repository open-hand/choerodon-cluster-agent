	...
	for _, h := range executingHooks {
		...
		if chartName != "choerodon-cluster-agent" {
			...
			for _, r := range resources {
				err = action.AddLabel(imagePullSecret, command, v1Command, appServiceId, v1AppServiceId, r, commit, chartVersion, releaseName, chartName, agentVersion, testLabel, namespace, isTest)
				if err != nil {
					return err
				}
			}
