...
func (crt *Certificate) GetCommitId() string {
	commitId := crt.ObjectMeta.Annotations[CertificateCommitIdKey]
	return commitId
}

func (crt *Certificate) FormatMessage(msg string) string {
	return fmt.Sprintf("[%s] %s", crt.GetCommitId(), msg)
}

func (crt *Certificate) FormatMessagef(msg string, args ... interface{}) string {
	s := fmt.Sprintf(msg, args...)
	return fmt.Sprintf("[%s] %s", crt.GetCommitId(), s)
}

func (crt *CertificateSpec) IsImport() bool {
	if crt.ExistCert.Key == "" || crt.ExistCert.Cert == "" {
		return false
	}
	return true
}

func (e *ExistCert) IsImport() bool {
	if e.Cert == "" || e.Key == "" {
		return false
	}
	return true
